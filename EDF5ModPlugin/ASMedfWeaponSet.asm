...
extern CustomMissionData_GetCustomFactor : proto
...
ASMreadWeaponSgoNode proc
	...
	; now, let's first check to see if adjusted by mission
	ReloadIsSupportBlock:
		lea rdx, wReloadIsSupport
		mov rcx, r14
		call edf5BDF30Address
		movsxd rcx, eax
		cmp ecx, -1
		je ReloadInitBlock ; if node does not exist, jump
		mov rax, [r14]
	...
	ofs38E30B:
		...
		xorps xmm0, xmm0
		movups [rbp+200h], xmm0
		mov r8, r12
		lea rcx, [rbp+200h] ; note that it cannot be replaced here
