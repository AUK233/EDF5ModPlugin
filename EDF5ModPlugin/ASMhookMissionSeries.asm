...
ASMgetCurrentMissionClassModelType proc

		mov rcx, vedf125AB30
		mov rax, [rcx]
		test rax, rax
		je returnValue
		mov r8, [rax+178h] ; mission sgo pointer
		movsxd rcx, dword ptr [rax+48h] ; mission index
		test r8, r8
		je returnValue
		cmp [r8+4], ecx
		jbe returnValue
		...
		add rax, r8
		lea rdx, [rax+rcx*4]
		movsxd rax, dword ptr [rax+rcx*4+8]
		add rdx, rax
		; is 3rd node, 2x12=24
		mov ecx, [rdx+24] ; get node type
		cmp ecx, 1
		jne checkIsFloat
		mov eax, [rdx+24+8] ; get node value
		ret
	checkIsFloat:
		cmp ecx, 2
		jne returnValue
		cvttss2si eax, dword ptr [rdx+24+8]
		ret
	returnValue:
		xor eax, eax
		ret
...
ASMgetCurrentMissionClassModelType ENDP
